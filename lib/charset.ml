...
external array_unsafe_get : ('a : any). 'a array -> int -> 'a = "%array_unsafe_get"
[@@layout_poly] [@@noalloc]

external iarray_unsafe_get : ('a : any). 'a iarray -> int -> 'a = "%array_unsafe_get"
[@@layout_poly] [@@noalloc]
...
let of_list cs =
  ...
  List.iter cs ~f:(fun c ->
    let ix = array_ix c in
    assert (ix <= 3 && ix >= 0);
    array_unsafe_set arr ix (Int64_u.logor (bit c) (array_unsafe_get arr ix)));
  Iarray.unsafe_of_array arr
;;

let mem t c =
  let bits = iarray_unsafe_get t (array_ix c) in
